...
# Prebuilt version of ghc that will be used to recompile itself.
BIN_PV=8.0.2
BIN_P="${PN}-${BIN_PV}"
# sorted!
...
relocate_ghc() {
	...
	local gp_back="${T}/ghc-pkg-${BIN_PV}-orig"
	cp "${WORKDIR}/usr/bin/ghc-pkg-${BIN_PV}" "$gp_back" || die "unable to backup ghc-pkg wrapper"
	...
	if [[ ${bin_libdir} != $(get_libdir) ]]; then
		...
		relocate_path "/usr/${bin_libdir}" "/usr/$(get_libdir)" \
			...
			"${WORKDIR}/usr/bin/ghci-${BIN_PV}" \
			"${WORKDIR}/usr/bin/ghc-pkg-${BIN_PV}" \
			"${WORKDIR}/usr/bin/hsc2hs" \
			"${WORKDIR}/usr/bin/runghc-${BIN_PV}" \
			"$gp_back" \
	...
	relocate_path "/usr" "${to}/usr" \
		...
		"${WORKDIR}/usr/bin/ghci-${BIN_PV}" \
		"${WORKDIR}/usr/bin/ghc-pkg-${BIN_PV}" \
		"${WORKDIR}/usr/bin/haddock-ghc-${BIN_PV}" \
		"${WORKDIR}/usr/bin/hp2ps" \
		"${WORKDIR}/usr/bin/hpc" \
		"${WORKDIR}/usr/bin/hsc2hs" \
		"${WORKDIR}/usr/bin/runghc-${BIN_PV}" \
		"${WORKDIR}/usr/$(get_libdir)/${BIN_P}/package.conf.d/"*
