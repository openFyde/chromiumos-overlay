...
def update_third_party_crates_ebuild_contents(contents: str,
                                              subsumed_ebuild_stem: str,
    ...
    return "\n".join(third_party_crates_lines[:rdepend_index + 1] +
                     rdepend_constraints +
...
def emptify_ebuild_contents(ebuild_contents: str) -> str:
    ...
    if i is None:
        ...
        if num_quotes == 1:
            ...
            ebuild_lines = (ebuild_lines[:i] + depend_lines +
                            ebuild_lines[i + 1:])
...
def main(argv: List[str]):
    ...
    crate_name = ebuild_path.parent.name
    if (crate_name not in migration_utils.CUSTOMIZATION_IGNORE_CRATES
            and migration_utils.crate_has_customization(ebuild_contents)):
