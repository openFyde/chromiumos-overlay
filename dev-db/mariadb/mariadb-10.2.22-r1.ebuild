...
pkg_setup() {
	...
	# Use the GID/UID as defined in the profile rather than the eclass.
	enewgroup mysql 245 || die "problem adding 'mysql' group"
	enewuser mysql 245 -1 -1 mysql || die "problem adding 'mysql' user"
}
...
src_configure(){
	...
	if use server ; then
		...
		mycmakeargs+=(
			...
			-DCMAKE_CXX_FLAGS="-I${SYSROOT}/usr/include -L${SYSROOT}/usr/$(get_libdir)"
		)
	...
	#Added to enable cross compile, this is based on the instructions here:
	...
	mycmakeargs+=(
		-DIMPORT_EXECUTABLES="${S}/host/import_executables.cmake"
	)
