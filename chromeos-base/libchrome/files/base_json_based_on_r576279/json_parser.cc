...
bool JSONParser::EatComment() {
  ...
  } else if (comment_start == StringPiece("/*")) {
...
Optional<Value> JSONParser::ConsumeDictionary() {
  ...
  Optional<DictionaryValue> dict = DictionaryValue();
  ...
  while (token != T_OBJECT_END) {
    ...
    dict->SetWithoutPathExpansion(key.DestructiveAsString(),
  ...
  return Optional<Value>(std::move(*dict));
}
...
Optional<Value> JSONParser::ConsumeList() {
  ...
  Optional<ListValue> list = ListValue();
  ...
  while (token != T_ARRAY_END) {
    ...
    list->Append(std::make_unique<Value>(std::move(*item)));
  ...
  return Optional<Value>(std::move(*list));
}
